...
void one_step_test(TestEnvironment& env, Entity& entity) {
  ...
  SOLARUS_ASSERT(entity.get_xy() - old_xy == Point(8, 0),
      "Unexpected coordinates for 'one_step_test #1'");
  SOLARUS_ASSERT(movement->get_total_distance_covered() == 8,
  ...
  SOLARUS_ASSERT(entity.get_xy() == old_xy,
      "Unexpected coordinates for 'one_step_test #2'");
  SOLARUS_ASSERT(movement->get_total_distance_covered() == 16,
  ...
  SOLARUS_ASSERT(entity.get_xy() - old_xy == Point(-8, -8),
      "Unexpected coordinates for 'one_step_test #3'");
  SOLARUS_ASSERT(movement->get_total_distance_covered() == 24,
  ...
  SOLARUS_ASSERT(movement->get_total_distance_covered() == 32,
...
void direction_test(TestEnvironment& env, Entity& entity) {
  ...
  SOLARUS_ASSERT(movement->get_current_direction() == 5,
...
void multi_step_test(TestEnvironment& env, Entity& entity) {
  ...
  SOLARUS_ASSERT(entity.get_xy() - old_xy == Point(0, 16),
  ...
  SOLARUS_ASSERT(movement->get_total_distance_covered() == 40,
...
void snap_test(TestEnvironment& env, Entity& entity) {
  ...
  SOLARUS_ASSERT(!entity.is_aligned_to_grid(),
  ...
  SOLARUS_ASSERT(entity.get_top_left_xy() == Point(147, 124),
  ...
  SOLARUS_ASSERT(entity.get_top_left_xy() == Point(152, 112),
